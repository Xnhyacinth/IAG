...
class ImageLitModel(pl.LightningModule):
    ...
    def add_model_specific_args(parent_args):
        ...
        parser.add_argument("--lr_scheduler_type", type=str, default="linear", help="scheduler_type of lr")
        
        parser.add_argument("--weight_decay", type=float, default=0.1)

        parser.add_argument('--hylora', action='store_true', help='hylora or not')
        ...
        parser.add_argument("--hidden_adapter_dim", type=int, default=768, help='Input dimension of adapter')
        parser.add_argument("--load_hypernet_weights", type=int, default=None, help='Path to hypernet weights, otherwise random init')
        parser.add_argument('--do_distill', action='store_true', help='distill or not')
        ...
        parser.add_argument("--alpha_kd", type=float, default=1.0, help="The weight of kd loss")
        ...
        # teacher setting
        parser.add_argument("--teacher_model", type=str, default="none", help="Path of teacher model")
        parser.add_argument("--train_teacher", action="store_true", help="Train teacher or not")
        parser.add_argument("--t_alpha_kd", type=float, default=0.4, help="The weight of kd loss if train_teacher is True")
        parser.add_argument("--t_learning_rate", type=float, default=3e-5, help="The learning rate of teacher")
        parser.add_argument("--n_context", type=int, default=10)
        parser.add_argument("--use_kl", action="store_true", help="Whether use kl")
        parser.add_argument("--use_attn", action="store_true", help="Whether use attn")
        parser.add_argument("--use_hidden", action="store_true", help="Whether use hidden")
        return parent_args
    ...
    def cal_loss(self, s_logits, t_logits, temperature):
        ...
        ori_kld_loss = (
            -torch.exp(soft_labels) * log_prob + torch.exp(soft_labels) * soft_labels
        )
    ...
    def training_step(self, batch, batch_idx):
        if self.args.do_distill:
            if self.args.use_lgtm:
                ...
                (idx, labels, _, context_ids, context_mask, t_context_ids, t_context_mask, features, _) = batch
                self.model.eval()
                self.t_model.train()
                with torch.no_grad():
                    outputs = self.model(
