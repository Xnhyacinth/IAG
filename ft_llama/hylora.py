...
from model import LlamaLoraWrapper
...
# class DataCollatorForSeq2Seqhylora(transformers.DataCollatorForSeq2Seq):
#     def __init__(self, tokenizer, pad_to_multiple_of=8, return_tensors="pt", padding=True):
#         super().__init__(tokenizer, pad_to_multiple_of, return_tensors, padding)
...
class ModelArguments:
    ...
    train_name: Optional[str] = field(
        default=None, metadata={"help": "train name"}
    )
...
def train():
    ...
    def generate_and_tokenize_prompt(data_point, add_eos_token=True):
        ...
        tokenized_full_prompt["features"] = data_point["features"]
        return tokenized_full_prompt

    def load_features(dataset, hg_datapath, use_context=False, n_c=5):
        hg_datapath = hg_datapath[0]
    ...
    raw_datasets = load_features(raw_datasets, data_args.validation_files)
    print(raw_datasets)
    ...
    if model_args.train_name == 'lora':
        ...
        device = model.device
        model = LlamaLoraWrapper(model, 32, None).to(device)
